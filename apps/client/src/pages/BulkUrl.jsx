...
export default function BulkUrl() {
    ...
    // Fetch domains and campaigns from Supabase
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (domainsData) {
                    ...
                    const activeDomain = formatted.find(d => d.status === 'Active');
                    if (activeDomain) {
    ...
    // ... (keep availableNetworks and auto-select effect detailed below in diff if needed, but they are derived from state so likely fine to keep as is if state structure is same)
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setProgress({ current: numberOfLinks, total: numberOfLinks });

        } catch (error) {
            console.error('Error in bulk generation:', error);
            // If bulk fail, show error for all? Or handled differently?
            // Simple generic error for now
