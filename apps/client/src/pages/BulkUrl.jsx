...
export default function BulkUrl() {
            ...
            try {
                ...
                if (response.ok) {
                    allProcessedResults.push({
                        original: targetUrl,
                        shortLink: `https://${selectedDomain}/${clickId}`,
                        status: 'Active',
                        error: null
                    });
                } else {
    ...
    return (
        <div className={`font-display flex flex-col min-h-screen overflow-y-auto transition-colors ${isDark ? 'bg-background-dark text-white' : 'bg-gray-50 text-gray-900'}`}>
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 flex flex-col gap-6">
                <GeneratorNavigation />
                {/* Configuration Panel */}
                <div className="w-full max-w-5xl mx-auto flex flex-col gap-6">
                    {/* Settings Card */}
                    <div className={`rounded-xl p-6 shadow-xl border transition-colors ${isDark ? 'bg-surface-dark border-border-dark' : 'bg-white border-gray-200'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            <label className="flex flex-col gap-2">
                                <span className={`text-sm font-medium ${isDark ? 'text-[#9293c9]' : 'text-gray-500'}`}>Target Network</span>
                                <div className="relative">
                                    <select
                                        value={selectedNetwork}
                                        onChange={(e) => setSelectedNetwork(e.target.value)}
                                        className={`w-full appearance-none rounded-lg border px-4 py-3 pr-10 focus:outline-none focus:ring-1 focus:ring-primary transition-all ${isDark ? 'bg-[#111122] border-[#323367] text-white focus:border-primary' : 'bg-gray-100 border-gray-200 text-gray-900 focus:border-primary'}`}
                                    >
                                    ...
                                    <div className={`absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none ${isDark ? 'text-[#9293c9]' : 'text-gray-400'}`}>
                                        <span className="material-symbols-outlined">expand_more</span>
                                    </div>
                                </div>
                            </label>
                            <label className="flex flex-col gap-2">
                                <span className={`text-sm font-medium ${isDark ? 'text-[#9293c9]' : 'text-gray-500'}`}>Tracking Domain</span>
                                <div className="relative">
                                    <select
                            ...
                            <label className="flex flex-col gap-2">
                                <span className={`text-sm font-medium ${isDark ? 'text-[#9293c9]' : 'text-gray-500'}`}>Number of Links</span>
                                <input
                                    type="number"
                                    min="1"
                                    max="100"
                                    ...
                                    className={`w-full rounded-lg border px-4 py-3 focus:outline-none focus:ring-1 focus:ring-primary transition-all ${isDark ? 'bg-[#111122] border-[#323367] text-white focus:border-primary placeholder:text-[#323367]' : 'bg-gray-100 border-gray-200 text-gray-900 focus:border-primary placeholder:text-gray-400'}`}
                                    placeholder="1"
                                />
                                <span className={`text-xs ${isDark ? 'text-[#9293c9]' : 'text-gray-500'}`}>Generate multiple links (max 100)</span>
                            </label>
                        </div>
                    </div>
