...
function AppContent() {
  ...
  return (
    <div className={`min-h-screen flex flex-col overflow-x-hidden transition-colors ${isDark ? 'bg-transparent text-white' : 'bg-gray-50 text-gray-900'}`}>
      <Background />
      {/* Show Header only on admin pages (not login, not generator) */}
      {isAdminPage && <Header />}

      <main className={`flex-grow w-full relative z-10 ${isAdminPage ? 'px-6 py-8' : ''}`}>
        <div className={isAdminPage ? "max-w-[1440px] mx-auto flex flex-col h-full" : "w-full h-full"}>
          <Routes>
            ...
            <Route element={<TrackerAuthGuard />}>
              ...
              <Route path="/:trackerId/geo-redirect" element={<GeoRedirect />} />
