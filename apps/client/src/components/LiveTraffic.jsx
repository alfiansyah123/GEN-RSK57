...
const LiveTraffic = () => {
    ...
    const fetchClicks = useCallback(async () => {
        ...
        try {
            const { data, error } = await supabaseTraffic
                .from('clicks')
                .select('id, slug, country, ip_address, created_at, click_id, os, browser, s3')
    ...
    const getOSIcon = (os) => {
        const iconStyle = { width: '12px', height: '12px', opacity: 0.8 };
    ...
    const getBrowserIcon = (browser) => {
        const iconStyle = { width: '12px', height: '12px', marginLeft: '3px', opacity: 0.8 };
    ...
    if (loading) {
        return (
            <div className="rounded-xl p-3 bg-white dark:bg-gray-800 border boder-gray-200 dark:border-gray-700 shadow-sm">
    ...
    const formatDisplayName = (click) => {
        // Priority: s3 (DANCOK) > click_id (truncated) > slug
        if (click.s3) return click.s3;
        if (click.clickId) {
            if (click.clickId.length > 20) return click.clickId.substring(0, 8) + '...';
            return click.clickId;
        }
        return `/${click.slug || 'Unknown'}`;
    };

    return (
        <div className="rounded-xl p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm transition-colors duration-200">
            ...
            <div className="flex flex-col gap-1 max-h-[350px] overflow-y-auto pr-1">
                {clicks.length === 0 ? (
                    ...
                    clicks.map((click) => (
                        <div
                            key={click.id}
                            className="flex items-center gap-2 p-2 rounded bg-gray-50 dark:bg-gray-700/30 hover:bg-gray-100 dark:hover:bg-gray-700 border border-transparent hover:border-gray-200 dark:hover:border-gray-600 group transition-all"
                        >
                            ...
                            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400 break-all leading-tight" title={click.clickId || click.slug}>
