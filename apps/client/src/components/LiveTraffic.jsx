...
const LiveTraffic = () => {
    ...
    const fetchClicks = useCallback(async () => {
        try {
            const { data, error } = await supabase
                .from('click')
                .select(`
                    id, ip, country, userAgent, referrer, os, browser, external_id, createdAt,
            ...
            if (data && mountedRef.current) {
                const formatted = data.map(row => ({
                    id: row.id,
                    slug: row.link?.trackerId || 'Unknown',
                    country: row.country,
                    ip: row.ip,
                    time: row.createdAt,
                    os: row.os || '',
                    browser: row.browser || '',
                    clickId: row.link?.trackerId || row.external_id || 'Unknown',
                    referrer: row.referrer || '',
                    network: row.link?.network || '',
                }));
                setClicks(formatted);
            }
        } catch (err) {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                'postgres_changes',
                { event: 'INSERT', schema: 'public', table: 'click' },
                async (payload) => {
                    if (!mountedRef.current) return;
                    const newClick = payload.new;

                    // Fetch related link info
                    let linkData = null;
                    try {
    ...
    const getOSIcon = (os) => {
        ...
        const linuxSvg = <svg viewBox="0 0 448 512" fill="#FCC624" style={iconStyle}><path d="M220.6 65.3c2.9-2.2 4.6-5.9 4.3-9.6-1.5-16.7 3.9-39 20-53 2.1-1.9 5.2-2.3 7.7-1.1 25.1 12.1 43.8 48 44.5 90.7.1 2.9-1.3 5.7-3.6 7.4-16.7 11.9-45.3 19.7-72.9-34.4z" /></svg>;
    ...
    const getBrowserIcon = (browser) => {
        ...
        const chromeSvg = <svg style={{ ...iconStyle, color: '#4285F4' }} viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C8.21 0 4.831 1.757 2.632 4.501l3.953 6.848A5.454 5.454 0 0 1 12 6.545h10.691A12 12 0 0 0 12 0zM1.931 5.47A11.943 11.943 0 0 0 0 12c0 6.012 4.42 10.991 10.189 11.864l3.953-6.847a5.45 5.45 0 0 1-6.865-2.29zm13.342 2.166a5.446 5.446 0 0 1 1.45 7.09l.002.001h-.002l-3.953 6.848c.538.068 1.085.107 1.64.107A12 12 0 0 0 24 12c0-.546-.046-1.082-.12-1.607zM12 16.364a4.364 4.364 0 1 1 0-8.728 4.364 4.364 0 0 1 0 8.728z" /></svg>;
        const safariSvg = <svg style={{ ...iconStyle, color: '#00A4E4' }} viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm0 21.6c-5.302 0-9.6-4.298-9.6-9.6s4.298-9.6 9.6-9.6 9.6 4.298 9.6 9.6-4.298 9.6-9.6 9.6zm1.146-13.854l-5.694 10.392 10.392-5.694-4.698-4.698z" /></svg>;
        ...
        if (lowerBrowser.includes('edge')) return <span style={{ fontSize: '12px' }}>🌐</span>;
        if (lowerBrowser.includes('opera')) return <span style={{ fontSize: '12px' }}>🔴</span>;
        if (lowerBrowser.includes('samsung')) return <span style={{ fontSize: '12px' }}>📱</span>;
            ...
            <div className="flex flex-col gap-1 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                {clicks.length === 0 ? (
                    ...
                    clicks.map((click) => (
                        <div
                            key={click.id}
                            className="flex items-center gap-2 p-2 rounded bg-gray-50 dark:bg-gray-700/30 hover:bg-gray-100 dark:hover:bg-gray-700 border border-transparent hover:border-gray-200 dark:hover:border-gray-600 group transition-colors"
                        >
                            ...
                            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                <span className="text-[10px] font-medium text-blue-600 dark:text-blue-400 break-all leading-tight" title={click.clickId}>
                                    {click.clickId}
                                </span>
