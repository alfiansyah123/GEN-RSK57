...
if ($path !== '/' && !preg_match('/\.(html|css|js|png|jpg|jpeg|gif|ico|svg)$/', $path)) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM link WHERE slug = $1");
        $stmt->execute([$slug]);
        $link = $stmt->fetch();
...
// If file exists (css, js), serve it.
$localFile = __DIR__ . $path;
if (file_exists($localFile) && !is_dir($localFile)) {
    // Optimization for PHP Built-in Server: Return false allows native handling
    if (php_sapi_name() === 'cli-server') {
        return false;
    }
    
    // For Apache/Production (if .htaccess fails for some reason)
    ...
    $mime = $mimes[$ext] ?? mime_content_type($localFile);
